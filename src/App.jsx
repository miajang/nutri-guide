  ...
  return (
    <div style={{background:"#fff",borderRadius:12,marginBottom:16,overflow:"hidden",boxShadow:"0 2px 6px rgba(0,0,0,.05)"}}>
      <div onClick={onToggle} style={{padding:"15px 18px",cursor:"pointer",display:"flex",alignItems:"flex-start",justifyContent:"space-between",gap:12}}>
        ...
        <div style={{fontSize:".78rem",color:t.learn,fontWeight:600,whiteSpace:"nowrap",flexShrink:0,marginTop:2}}>{isOpen?"Show less \u25B2":"Learn more \u25BC"}</div>
      </div>
      {isOpen&&(
...
const ChatPanel = ({isOpen,onClose,selConds,sex,age,relNutrients,t}) => {
  ...
  return (
    <div style={{position:"fixed",bottom:0,right:0,width:390,maxWidth:"100vw",height:"min(620px, 85vh)",background:"#fff",borderTopLeftRadius:16,boxShadow:"-4px -4px 24px rgba(0,0,0,.15)",zIndex:500,display:"flex",flexDirection:"column",overflow:"hidden",border:`1px solid ${t.mid}`}}>
      <div style={{padding:"14px 18px",background:t.pri,color:"#fff",display:"flex",justifyContent:"space-between",alignItems:"center",flexShrink:0}}>
        <div><div style={{fontWeight:700,fontSize:".92rem"}}>Ask NutriGuide</div><div style={{fontSize:".72rem",opacity:.8}}>{selConds.length} {"condition"}{selConds.length>1?"s":""} {"\u00B7"} Personalized AI Guidance</div></div>
        <button onClick={onClose} style={{background:"rgba(255,255,255,.2)",border:"none",color:"#fff",width:28,height:28,borderRadius:6,cursor:"pointer",fontSize:".85rem"}}>{"\u2715"}</button>
      </div>
        ...
        {msgs.length===0&&(
          <div>
            ...
            <div style={{display:"flex",flexDirection:"column",gap:6}}>
              {suggestions.map((s,i)=><button key={i} onClick={()=>setInput(s)} style={{background:t.lt,border:`1px solid ${t.mid}`,borderRadius:8,padding:"8px 12px",fontSize:".82rem",color:t.pri,cursor:"pointer",textAlign:"left",fontWeight:500}}>{s}</button>)}
            </div>
          </div>
        )}
        {msgs.map((m,i)=>(
...
export default function NutriGuide() {
  ...
  const NavItems = ({onExtra}) => (
    <>
      ...
      <div style={{marginTop:"auto",padding:"40px 16px 16px"}}>
        <div onClick={()=>{setChatOpen(true);onExtra?.();}} style={{display:"flex",alignItems:"center",gap:8,padding:"9px 12px",fontSize:".82rem",color:t.learn,cursor:"pointer",fontWeight:600,borderRadius:8,transition:"all .15s",letterSpacing:".02em"}}>
          <span style={{width:16,height:16,display:"flex",alignItems:"center",justifyContent:"center"}}><NavIcon type="chat" color={t.learn}/></span>
          Ask NutriGuide
        </div>
      </div>
    </>
    ...
    return (
      <div style={{fontFamily:"'Segoe UI',system-ui,sans-serif",background:"#fff",minHeight:"100vh",color:"#555"}}>
        ...
        <header style={{background:"#fff",borderBottom:"1px solid #e8eeec",padding:"14px 28px",display:"flex",alignItems:"center",justifyContent:"space-between",position:"sticky",top:0,zIndex:100}}>
          <div><div style={{fontSize:"1.2rem",fontWeight:800,color:t.pri}}>NutriGuide <span style={{color:"#777",fontWeight:400,fontSize:"1rem"}}>for Chronic Conditions</span></div><div style={{fontSize:".75rem",color:"#777",marginTop:2}}>Evidence-based, personalized nutrition guidance</div></div>
